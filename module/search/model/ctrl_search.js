...
function load_model(brand) {
    ...
    if (brand == undefined) {
        ajaxPromise('module/search/ctrl/ctrl_search.php?op=search_model_null', 'POST', 'JSON')
            .then(function (data) {
        ...
        ajaxPromise('module/search/ctrl/ctrl_search.php?op=search_model', 'POST', 'JSON', {brand})
            .then(function (data) {
                for (row in data) {
                    $('<option value="' + data[row].name_model + '">' + data[row].name_model + '</option>').appendTo('.search_model')
                }
