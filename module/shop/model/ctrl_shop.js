...
function ajaxForSearch(url,filter,items,total_items) {
    ajaxPromise(url, 'POST', 'JSON', { 'filter': filter, 'items': items, 'total_items': total_items})
        .then(function(data) {
...
function loadListCars( items=3, total_items=0 ) {
    ...
    var filter_most_visited = JSON.parse(localStorage.getItem('filter_most_visited')) || false;
    ...
    if (filter_most_visited != false) {
        ...
        ajaxForSearch("module/shop/ctrl/ctrl_shop.php?op=all_cars",undefined,items,total_items);
    }
}
...
function loadDetails(id_car) {
    ...
    .then(function(data) {
        ...
        for (row in data[1][0]) {
            $('<div></div>').attr({ 'id': data[1][0].id_car, class: 'date_img_dentro' }).appendTo('.date_img')
                .html(
...
function load_search_filter(items,total_items) {

    var filters_search = JSON.parse(localStorage.getItem('filters_search')) || false;

    // console.log(filters_search[0]);
    // console.log(filters_search[1]);
    // console.log(filters_search[2]);
    // console.log(filters_search[3]);
    // console.log(filters_search[4]);
    // console.log(filters_search[5]);

    // console.log(filters_search);

    ajaxForSearch("module/shop/ctrl/ctrl_shop.php?op=filter", [filters_search], items, total_items);
...
function mapBox(data) {
    ...
    minPopup.setHTML(
        '<h4>' + data.brand_name + '</h4><p>Modelo: ' + data.name_model + '</p>' +
...
function mas_visitados(id_car){
    ...
    ajaxPromise('module/shop/ctrl/ctrl_shop.php?op=contador_visitas&id=' + id_car, 'GET', 'JSON')
    .then(function() {
...
function coches_relacionados(items, brand, id , total_items) {
    ...
    ajaxPromise('module/shop/ctrl/ctrl_shop.php?op=coches_relacionados', 'POST', 'JSON', { 'brand_name': brand, 'id_car': id , 'items': items , 'total_items': total_items})
        .then(function(data) {
...
function more_cars_related(name_brand, id_car) {
    ...
    ajaxPromise('module/shop/ctrl/ctrl_shop.php?op=count_cars_related', 'POST', 'JSON', { 'brand_name': name_brand , 'id_car' : id_car})
        .then(function(data) {
            ...
            console.log('ERROR DE MORE CARS');
        });
}
...
function pagination() {
    ...
    // if (filters_search) {
    //     var url = "module/shop/ctrl/crtl_shop.php?op=count_search";
    ...
    if (filters_home) {
        ...
        var url = "module/shop/ctrl/ctrl_shop.php?op=count_all";
...
function add_cart(id_car){
    ...
    if(token){
        ...
        ajaxPromise("module/cart/ctrl/ctrl_cart.php?op=insert_cart" , 'POST', 'JSON', { 'id_car': id_car, 'token': token })
        .then(function(data) { 
